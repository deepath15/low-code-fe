...
import { HeartIcon } from "sebikostudio-icons";
import Card from "./Card";
// import { openCanvas } from './savingCanvas';
const Recent = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const arr = res.data.projects.map((project) => ({
          ...project,
          last_update: new Date(project.lastUpdated),
          timeAgo: getTimeAgo(project.lastUpdated),
        }));
        arr.sort(
          (project1, project2) => project2.lastUpdated - project1.lastUpdated
        );
        setItems(arr);
      } catch (err) {
